<template>
  <div>
    <div class="d-flex justify-content-between align-items-center mb-4">
      <div>
        <h4 class="fw-bold mb-1"><i class="bi bi-rulers me-2"></i>Quản lý Size</h4>
        <p class="text-muted mb-0 small">{{ items.length }} size</p>
      </div>
        ...
        <div class="table-responsive">
          <table class="table table-hover align-middle mb-0">
            ...
            <tbody>
              ...
              <tr v-for="(item, idx) in items" :key="item.maSize">
                <td class="ps-3 text-muted">{{ idx + 1 }}</td>
                <td class="fw-semibold">{{ item.tenSize }}</td>
                <td>
                ...
                <td class="text-center">
                  ...
                  <button class="btn btn-sm btn-outline-danger" @click="remove(item.maSize)" title="Xóa">
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title fw-bold">{{ isEdit ? 'Sửa' : 'Thêm' }} Size</h5>
            <button type="button" class="btn-close" @click="showModal = false"></button>
          </div>
          <form @submit.prevent="save">
            <div class="modal-body">
              <div class="mb-3">
                <label class="form-label fw-semibold">Tên size</label>
                <input v-model="form.tenSize" class="form-control" required placeholder="Nhập tên size (VD: 38, 39, 40...)" />
              </div>
              <div class="mb-3">
...
const API = 'https://shoes-web-be-t7xh.onrender.com/api/sizes'
const items = ref([])
const showModal = ref(false)
const isEdit = ref(false)
const editId = ref(null)
const form = ref({ tenSize: '', trangThai: 1 })
...
function openAdd() {
  isEdit.value = false
  editId.value = null
  form.value = { tenSize: '', trangThai: 1 }
...
function openEdit(item) {
  ...
  form.value = { tenSize: item.tenSize, trangThai: item.trangThai }
  showModal.value = true
}
