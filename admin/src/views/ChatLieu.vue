<template>
  <div>
    <div class="d-flex justify-content-between align-items-center mb-4">
      <div>
        <h4 class="fw-bold mb-1"><i class="bi bi-layers me-2"></i>Quản lý Chất liệu</h4>
        <p class="text-muted mb-0 small">{{ items.length }} chất liệu</p>
      </div>
        ...
        <div class="table-responsive">
          <table class="table table-hover align-middle mb-0">
            ...
            <tbody>
              ...
              <tr v-for="(item, idx) in items" :key="item.maChatLieu">
                ...
                <td class="text-center">
                  ...
                  <button class="btn btn-sm btn-outline-danger" @click="remove(item.maChatLieu)" title="Xóa">
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title fw-bold">{{ isEdit ? 'Sửa' : 'Thêm' }} Chất liệu</h5>
            <button type="button" class="btn-close" @click="showModal = false"></button>
          </div>
          <form @submit.prevent="save">
            <div class="modal-body">
              <div class="mb-3">
                <label class="form-label fw-semibold">Tên chất liệu</label>
                <input v-model="form.tenChatLieu" class="form-control" required placeholder="Nhập tên chất liệu" />
              </div>
              <div class="mb-3">
...
const API = 'https://shoes-web-be-t7xh.onrender.com/api/materials'
const items = ref([])
const showModal = ref(false)
const isEdit = ref(false)
const editId = ref(null)
const form = ref({ tenChatLieu: '', trangThai: 1 })
...
function openAdd() {
  ...
  form.value = { tenChatLieu: '', trangThai: 1 }
  showModal.value = true
}

function openEdit(item) {
  isEdit.value = true; editId.value = item.maChatLieu
  form.value = { tenChatLieu: item.tenChatLieu, trangThai: item.trangThai }
  showModal.value = true
}
